...
from input.models import MasterTrade, MasterLane, MasterPort, DefaultCurrentProforma
...
def master_trade(db):
    """마스터 Trade 데이터"""
    return MasterTrade.objects.create(
        trade_code='A1',
...
def master_lane(db):
    """마스터 Lane 데이터"""
    return MasterLane.objects.create(
        lane_code='AE01',
...
def master_trades(db):
    ...
    trades = []
    for i in range(1, 4):
        trades.append(MasterTrade.objects.create(
            trade_code=f'T{i}',
...
def master_lanes(db):
    ...
    lanes = []
    for i in range(1, 4):
        lanes.append(MasterLane.objects.create(
            lane_code=f'L{i:02d}',
            lane_name=f'Lane {i}'
        ))
    return lanes
...
def master_ports(db):
    ...
    ports = []
    for i in range(1, 11):
        ports.append(MasterPort.objects.create(
            port_code=f'P{i:03d}',
            port_name=f'Port {i}'
        ))
    return ports
...
def default_proformas(db, master_trades, master_lanes):
    ...
    for i, (trade, lane) in enumerate(zip(master_trades, master_lanes)):
        proformas.append(DefaultCurrentProforma.objects.create(
            trade=trade,
            lane=lane,
            capacity=(i + 1) * 1000,
            qty=(i + 1) * 500
...
def simulation_runs_by_user(db, user_a, user_b):
    ...
    user_a_sims = []
    for i in range(1, 6):
        user_a_sims.append(SimulationRun.objects.create(
    ...
    user_b_sims = []
    for i in range(6, 9):
        user_b_sims.append(SimulationRun.objects.create(
            simulation_number=f'{today}_{i:02d}-01',
            status='SUCCESS',
            created_by=user_b,
            base_simulation_number=None
...
def simulation_proformas(db, simulation_run, master_trades, master_lanes):
    ...
    for trade, lane in zip(master_trades, master_lanes):
        proformas.append(SimulationProforma.objects.create(
            simulation=simulation_run,
            trade=trade,
            lane=lane,
            capacity=1000,
