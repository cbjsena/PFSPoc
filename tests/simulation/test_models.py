...
class SimulationRunModelTests(TestCase):
    ...
    def test_sim_num_gen_001_simulation_number_format(self):
        ...
        sim = SimulationRun.objects.create(
            simulation_number="20260630_01-01", status="PENDING", created_by=self.user
        )
    ...
    def test_simulation_properties(self):
        ...
        running_sim = SimulationRun.objects.create(
            simulation_number="20260630_01-01", status="RUNNING", created_by=self.user
        )
        self.assertTrue(running_sim.is_running)

        # is_success 프로퍼티
        success_sim = SimulationRun.objects.create(
            simulation_number="20260630_02-01", status="SUCCESS", created_by=self.user
        )
        self.assertTrue(success_sim.is_success)

        # is_processing 프로퍼티
        pending_sim = SimulationRun.objects.create(
            simulation_number="20260630_03-01", status="PENDING", created_by=self.user
        )
...
class SimulationProformaModelTests(TestCase):
    ...
    def setUp(self):
        ...
        # Master 데이터 생성
        self.trade = MasterTrade.objects.create(trade_code="A1", trade_name="Asia Export")
        self.lane = MasterLane.objects.create(lane_code="AE01", lane_name="Asia-Europe 01")

    def test_simulation_proforma_creation(self):
        """SimulationProforma 생성"""
        proforma = SimulationProforma.objects.create(
            simulation=self.sim, trade=self.trade, lane=self.lane, capacity=1000, qty=500
        )
    ...
    def test_simulation_proforma_unique_constraint(self):
        ...
        with self.assertRaises(IntegrityError):
            SimulationProforma.objects.create(
                simulation=self.sim, trade=self.trade, lane=self.lane, capacity=2000, qty=1000
            )

    def test_simulation_proforma_optional_fields(self):
        """Capacity/Qty 선택사항"""
        proforma = SimulationProforma.objects.create(
            simulation=self.sim, trade=self.trade, lane=self.lane, capacity=None, qty=None
        )
