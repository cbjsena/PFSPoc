...
class TestPortCreationView:
    ...
    def test_sim_prt_dis_002_load_existing_simulation(self, api_client, simulation_run):
        ...
        response = api_client.get(
            reverse('simulation:port_creation'),
            {'sim_id': simulation_run.id}
        )
    ...
    def test_sim_prt_api_004_save_port_data_to_db(
        ...
        assert SimulationProforma.objects.filter(simulation=simulation_run).count() == len(valid_rows_data)
        # 변경된 값 확인
...
class TestSimulationNumberGeneration:
    ...
    def test_sim_num_gen_003_existing_simulation_number_preserved(
        self, api_client, simulation_run
    ):
